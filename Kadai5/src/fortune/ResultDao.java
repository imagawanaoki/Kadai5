...
public class ResultDao {
	...
	public static Map<String, Integer> unsei(Date uranai_date) throws ClassNotFoundException, SQLException {
		...
		for (int i = 1; i <= 6; i++) {
			...
			String sql = "Select COUNT(*) FROM result res INNER JOIN omikuji omi ON res.omikuji_id = omi.omikuji_id WHERE res.uranai_date >= ? AND res.uranai_date < ? AND omi.unsei_id = ? ";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setDate(1, last);
			preparedStatement.setDate(2, uranai_date);
			...
			rs.next();
			int uranaidate = rs.getInt("count");

			//運勢名を取得
			String sql2 = "SELECT unsei_name FROM UnseiMaster WHERE unsei_id = ?";
			PreparedStatement pr = connection.prepareStatement(sql2);
			pr.setInt(1, i);
			ResultSet rSet = pr.executeQuery();

			rSet.next();
			String fortune = rSet.getString("unsei_name");
	...
	public static Map<String, Integer> todayunsei(Date uranai_date) throws ClassNotFoundException, SQLException {
		...
		for (int i = 1; i <= 6; i++) {
			...
			//unsei_idと同じ運勢のレコードを呼び出す
			String sql = "Select COUNT(*) FROM result res INNER JOIN omikuji omi ON res.omikuji_id = omi.omikuji_id WHERE res.uranai_date = ? AND omi.unsei_id = ? ";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setDate(1, uranai_date);
	...
	public static Map<Date, String[]> birthdayresult(Date uranai_date, Date birthday)
		...
		String sql = "SELECT um.unsei_name, omi.gakumon, omi.negaigoto, omi.akinai, res.uranai_date FROM UnseiMaster um "
				+ "INNER JOIN Omikuji omi ON um.unsei_id = omi.unsei_id INNER JOIN result res ON omi.omikuji_id =res.omikuji_id WHERE res.uranai_date >= ? AND res.uranai_date <= ? AND res.birthday = ?";
